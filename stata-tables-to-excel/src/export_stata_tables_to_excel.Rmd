...
Before we can export returned results to Excel, we first need to use `putexcel set` to indicate which Excel workbook and sheet we’d like the results to be written to.[^7]
...
[^3]: See note 1.
[^4]: See note 2.
[^5]: For more information about returned results, see UCLA IDRE's FAQ [How can I access information stored after I run a command in Stata?](https://stats.idre.ucla.edu/stata/faq/how-can-i-access-information-stored-after-i-run-a-command-in-stata-returned-results/).
[^6]: Interested user can check out [Chapter 14, Matrix expressions](https://www.stata.com/manuals/u14.pdf) and section 18.3 of [Chapter 18, Programming Stata](https://www.stata.com/manuals/u18.pdf) in the Stata User's Guide.
[^7]: Stata version 14 or later is required to use `putexcel` with cell ranges, as we do in this example . To replicate the example in Stata 13, users will need to use loops. For instance, to replicate the line `putexcel A17:A21 = "Overall"`:
```{.stata}
forvalues c = 17/21 {
  putexcel A`c' = "Overall"
}
```
