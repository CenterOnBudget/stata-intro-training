...
```{stata, eval=FALSE, include=FALSE}
...
Stata's default confidence level is 95%, so that is the level used by estimation commands to compute the confidence interval. We can specify the option `level(90)` to an individual estimation command to indicate that we want that command to use a 90% confidence level (the Census Bureau's standard). Or, we can run `set level 90`, after which all estimation commands during the Stata session will use a 90% confidence level.
...
- The number of observations and weighted population count over which the estimate was computed (top right). 
...
Recall that estimation commands save all this information, and more, as "returned results" in `e()` and `r()`. We can see what the `svy: mean` command above stored by listing returned results:
...
Note that the by default, the table printed to the Results pane by estimation commands display numbers with more than 9 digits in scientific notation so that they fit on the screen. To see the total weight of the penguins in an easier-to-read format, we can use returned results. Recall that estimation commands store estimates in the matrix `e(b)`. We can use the `display` command to print the estimated total to the Results pane, specifying how many digits we'd like to see.
...
```{stata, include=FALSE, eval=FALSE}
...
The p-value of our hypothesis that male and female Chinstrap penguins have the same mean bill length is 0.0095, or 0.095%. If our desired confidence level is 90%, the critical p-value is 10%. Because the p-value of our hypothesis (0.095%) is less than the critical value (10%), we can say that the difference between the mean bill length of male and female penguins is statistically significant at a 90% confidence level.

The `test` command stores the p-value in `r()`:

```{stata}
return list
```

We can use `r(p)` to create a local macro that indicates whether `r(p)` exceeds the critical value for a 90% confidence level. We can then use `putexcel` to save the macro in the Excel spreadsheet where we are putting the results of our analysis
