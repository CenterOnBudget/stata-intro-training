...
Done! We now have a nice table in `results_custom.xlsx`. We could now open the file in Excel and manually customize the font and number formatting.[^5] 
...
We'll calculate the local macro `significant_diff` as `"significant"` if `r(p)` is greater or equal to 10%, and `"not significant"` if it is not. (The `cond(x, a, b)` function is like saying "if x is true, then a, otherwise b".) We can use `display` to print `significant_diff` to the results pane to see what it contains.
